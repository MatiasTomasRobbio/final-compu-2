...
def hand(player_list):
    ...
    # Initialize the pot
    pot = 0
    # Run the preflop betting round
    pot, players_in_hand, everyone_folds = betting_round(players_in_hand,player_list, pot)
    if everyone_folds == True:
        print(f"Todos los jugadores se retiraron.El jugador {players_in_hand[0]['name']} gana el bote.")
        return pot, players_in_hand[0]['name']
    # Deal the flop
    community_cards = deal_community_cards(deck,community_cards,3)
    print(community_cards)
    # Run the postflop betting round
    pot, players_in_hand, everyone_folds = betting_round(players_in_hand,player_list, pot)
    if everyone_folds == True:
    ...
    # Deal the turn
    community_cards = deal_community_cards(deck,community_cards,1)
    print(community_cards)
    # Run the postturn betting round
    pot, players_in_hand, everyone_folds = betting_round(players_in_hand,player_list, pot)
    if everyone_folds == True:
    ...
    community_cards = deal_community_cards(deck,community_cards,1)
    print(community_cards)
    # Run the postrive betting round
    pot, players_in_hand, everyone_folds = betting_round(players_in_hand,player_list, pot)
    if everyone_folds == True:
